...
$reponse = $bdd->query("SELECT p.uid,c.cmdid ,p.nom, p.description, c.qte,p.prix,c.statut FROM commande c INNER JOIN produits p on c.pid=p.pid WHERE c.statut='acceptee' AND p.uid=' ".$_SESSION['uid_vendeur']." ' ");
$somme_produit=0;
...
while ($donnees = $reponse->fetch()){ 
  $prix_total=$donnees['qte']*$donnees['prix'];
    ...
    echo"<div class='center' > <h4>Le nombre de Commandes acceptées est: $nb_commande </h4> </div>";
    echo"<div class='center' > <h4>Le chiffre d'affaire jusqu'à présent (montant de tous les produits) est de: $somme_produit €</h4> </div>";
$reponse->closeCursor();
}
catch(Exception $e)
{
